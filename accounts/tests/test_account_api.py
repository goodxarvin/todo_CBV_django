...
class TestAccountAPI:
    ...
    def test_new_password_forgot_200(self, api_client, user_object):
        str_access_token = str(RefreshToken.for_user(user_object).access_token)
        url = reverse("accounts:all-api-urls:account-urls:new-password", kwargs={"reset_pass_jwt": str_access_token})
        data = {
            "new_pass": "string123",
            "confirm_pass": "string123",
        }
    ...
    def test_verify_account_status_200(self, api_client, user_object):
        str_access_token = str(RefreshToken.for_user(user_object).access_token)
        url = reverse("accounts:all-api-urls:account-urls:verify-account", kwargs={"access_jwt": str_access_token})
        response = api_client.get(url)
        assert response.status_code == 200

    def test_verify_account_status_400(self, api_client, user_object):
        # str_access_token = str(RefreshToken.for_user(user_object).access_token)
        url = reverse("accounts:all-api-urls:account-urls:verify-account", kwargs={"access_jwt": "test_400"})
        response = api_client.get(url)
        assert response.status_code == 400
